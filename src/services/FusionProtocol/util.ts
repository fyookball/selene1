...
export function genKeypair(): [Uint8Array, Uint8Array, Uint8Array] {
  // secretKey: 32-byte random scalar < n
  const privkey = secp.utils.randomPrivateKey(); // Uint8Array(32)

  // Compressed public key (33 bytes)
  const pubkeyCompressed = secp.getPublicKey(privkey, true); // compressed

  // Uncompressed public key (65 bytes, prefix 0x04)
  const pubkeyUncompressed = secp.getPublicKey(privkey, false); // uncompressed
